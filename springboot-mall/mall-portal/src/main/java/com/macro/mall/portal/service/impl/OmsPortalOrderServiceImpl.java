...
//    }
    ...
    @Override
    public void confirmsOrder(Long orderId) {
        OmsOrder order = new OmsOrder();
        order.setId(orderId);
...
//        cancelOrderSender.sendMessage(orderId, delayTimes);
    ...
    public ConfirmOrderResult submitPreview(OrderParam orderParam) {
        ...
        if ("1".equals(type)) {
            ...
            String cart_id_list1 = orderParam.getCartId();
            String[] ids1 = cart_id_list1.split(",");
            List<Long> resultList = new ArrayList<>(ids1.length);
            for (String s : ids1) {
                resultList.add(Long.valueOf(s));
    ...
    public int payOrder(TbThanks tbThanks) {
        ...
        //设置订单为已付款
        OmsOrder tbOrder = orderMapper.selectByPrimaryKey(tbThanks.getOrderId());
        if (tbOrder == null) {
        ...
        if (orderMapper.updateByPrimaryKeySelective(tbOrder) != 1) {
            throw new ApiRRException("更新订单失败");
        }
        //恢复所有下单商品的锁定库存，扣减真实库存
        OmsOrderDetail orderDetail = portalOrderDao.getDetail(tbOrder.getId());
        int count = portalOrderDao.updateSkuStock(orderDetail.getOrderItemList());
        //发送通知确认邮件
